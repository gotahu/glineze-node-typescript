...
function renderCountdownMessage(template: string, title: string, days: number): string {
  ...
  return template.replace(
    /{{\s*(title|days)\s*}}|{\s*(title|days)\s*}/g,
...
function isTodayTargetDate(): boolean {
  ...
  const countdownDays = config
    .getConfig('countdown_notify_days')
    .split(',')
...
async function forceSendCountdownMessage(services: Services): Promise<void> {
  ...
  const message = renderCountdownMessage(
    config.getConfig('countdown_message'),
    eventTitle,
    daysLeft
  );
